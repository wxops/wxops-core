...
> producer-side contract lives in [`status-contract.md`](../api-reference/status-contract.md)
...
1. **Pin `v1alpha1`.** All seven kinds serve only `v1alpha1` and will until
   the OSS release (decision recorded in [`VERSIONS.yaml`](../../VERSIONS.yaml) and [status-contract.md](../api-reference/status-contract.md#api-version)).
...
| **Monitoring toggle** | `parameters.monitoring.enabled` | **Repoint the existing toggle here** — the old `prometheus.io/*` annotations never worked ([observability.md](../core-ideas/observability.md)). `kind: auto` is the right default; only surface the explicit choice when `service.enabled` is known, since `ServiceMonitor` without a Service is a render-time error |
...
[`self-service-operations.md`](../core-ideas/self-service-operations.md) — when
...
| "Open shell" | requires an RBAC binding to `status.darlane.serviceAccountName` — the portal *displays* the SA name and links the runbook; it does not grant access (the RBAC seam — [tenant-app.md](../api-reference/tenant-app.md#developer-access-and-rbac)) |
...
[`knowledge-architecture.md`](../core-ideas/knowledge-architecture.md#runbook-anatomy--the-format-worth-standardizing)):
...
attached ([self-service-operations.md](../core-ideas/self-service-operations.md#the-error-taxonomy--developer-side-user-side-platform-side)).
...
- Fan an XR out to multiple clusters itself by mutating one XR — multi-region
  ...
  ([multi-cluster-proposal.md](../core-ideas/multi-cluster-proposal.md)); the portal owns
...
[`status-contract.md`](../api-reference/status-contract.md) — the producer contract ·
[`tenant-app.md`](../api-reference/tenant-app.md) / [`tenant-database.md`](../api-reference/tenant-database.md)
— full field reference · [`solution-matrix.md`](../core-ideas/solution-matrix.md) — what's
