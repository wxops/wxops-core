...
> plainly. It is the content basis for the `SECURITY.md` still open in
> ROADMAP Phase 4, and the review artifact for the OSS release.
...
- **B1 — who may act**: human identity via Pinniped (Gitea OIDC); agent
  ...
  (`ROADMAP.md` §Decided and rejected; the seam in [tenant-app.md](tenant-app.md#developer-access-and-rbac)).
...
2. **No RBAC from compositions** + **the SA-in-status seam** — `ROADMAP.md` §Decided and rejected; [tenant-app.md](tenant-app.md#developer-access-and-rbac).
...
2. **`SECURITY.md`** (disclosure channel) — Phase 4 item; this doc is its content.
