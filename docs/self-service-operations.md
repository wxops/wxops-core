...
> [`status-contract.md`](status-contract.md) (the first diagnostic surface),
...
— see [`status-contract.md`](status-contract.md). Before any dashboard is
...
1. **The status contract** — a stable, documented, machine-readable "where
   does it hurt" for every kind ([`status-contract.md`](status-contract.md)).
...
- [`status-contract.md`](status-contract.md) · [`observability.md`](observability.md) — Pillar 1's foundations
