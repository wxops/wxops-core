...
| ADR | `ROADMAP.md` §"Decided and rejected" is the decision log — decision + why, marked "do not re-litigate". [`docs/adr/`](../adr/README.md) now holds the permanent record itself, one committed file per decision (started with [ADR-001](../adr/001-package-channel-label.md)) | Convention: new decisions are proposed as **ADR issues** (`.github/ISSUE_TEMPLATE/adr.md`); once accepted, the issue becomes the discussion trail and a `docs/adr/NNN-title.md` file becomes the permanent record — a deliberate small reversal of "no parallel file tree," because a committed file outlives and out-diffs an issue |
...
| Reference | [`status-contract.md`](../api-reference/status-contract.md), per-package docs, [`tests/README.md`](../../tests/README.md) | Healthy — Phase 0 built this deliberately |
...
- [`status-contract.md`](../api-reference/status-contract.md) — the semantic layer's anchor; `CLAUDE.md` — the existing proof of agent-facing operational memory
