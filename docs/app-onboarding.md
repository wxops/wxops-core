...
a running [`tenant-app`](tenant-app.md), tying together the templates
catalog, [`gitea-repository`](gitea-repository.md), and `tenant-app`'s
...
The portal creates an [`XGiteaRepository`](gitea-repository.md) claim:
...
[`XTenantApp`](tenant-app.md) claim:
...
If the app needs a database, create a [`tenant-database`](tenant-database.md)
claim first (or alongside), then set
`secretsFrom.database.enabled: true` on the `tenant-app` claim — see
[tenant-app's go-live ordering contract](tenant-app.md#vault-secrets--databases)
...
| 3 | Create new app repo | [`gitea-repository`](gitea-repository.md) (skip if importing an existing repo) |
...
| 8 | Application workload | [`tenant-app`](tenant-app.md) |
| 9 | Database + secrets (optional) | [`tenant-database`](tenant-database.md) + platform `ExternalSecret`/`PushSecret` |
